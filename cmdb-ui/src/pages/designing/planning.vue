...
import { getPlanningDesignsCiData, getCiTypeAttributes, deleteCiDatas, createCiDatas, updateCiDatas, getEnumCodesByCategoryId, getPlanningDesignTabs, getAllIdcDesignData, getIdcDesignTreeByGuid, getAllZoneLinkDesignGroupByIdcDesign, operateCiState } from '@/api/server'
...
export default {
  ...
  methods: {
    ...
    genDOT (idcData, linkData) {
      ...
      let width = 16
      let height = 12
      let dots = ['digraph G {', 'rankdir=TB nodesep=0.5;', `node [shape="box", fontsize="${fsize}", labelloc="t", penwidth="2"];`, `subgraph cluster_${idcData.data.guid} {`, `style="filled";color="${colors[0]}";`, `label="${idcData.data.name || idcData.data.description || idcData.data.code}";`, `size="${width},${height}";`, this.genChildren(idcData), this.genLink(links), '}}']
      return dots.join('')
    },
    genChildren (idcData) {
      const width = 16
      ...
      if (layers.size) {
        ...
        dots.push(`g_${idcData.data.guid}[label=" ";color="${colors[0]}";width="${width - 0.5}";height="${height - 3}"]`)
      }
      return dots.join('')
    },
    genLink (links) {
      ...
      if (pw > ph * 1.2) {
        ...
        for (var p = 0; p < n; p++) {
          ...
          g.append('text')
            ...
            .attr('font-size', fontsize)
          if (Array.isArray(node.children[p].children)) {
            this.setChildren(node.children[p], { x: rx, y: ry }, w, h, fontsize, deep + 1, idcName)
          }
        }
      }
    },
      ...
      if (statusCode === 'OK') {
        ...
        if (idcLink && idcLink.linkList) {
          idcLink.linkList.forEach(_ => {
            let zoneLink = {}
            if (_.data.network_zone_design_1.data_center_design === this.selectedIdc && _.data.network_zone_design_2.data_center_design === this.selectedIdc) {
