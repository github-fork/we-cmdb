  ...
  <div>
    <Row>
      ...
      <Col span="10" offset="1">
        ...
        <Select v-model="selectedQuery.id" filterable style="width: 75%;" :disabled="!selectedCI.id" @on-change="onQueryNameSelectChange">
          <Option v-for="(item, index) in queryNameList" style="overflow:hidden;line-height:24px;" :value="item.id" :key="item.id"
            >{{ item.name }}
            <span v-if="index === 0">
              <Button @click.stop.prevent="createIntQuery(item)" v-if="index === 0" icon="md-add" type="success" long></Button>
            </span>
      ...
      <Col span="6" offset="1">
        ...
        <Modal v-model="newGraphNameModalVisible" :title="$t('add_integrated_query_name')" @on-ok="saveGraph" @on-cancel="() => {}">
...
<script>
import { getAllCITypes, getQueryNames, fetchIntQueryById, saveIntQuery, updateIntQuery, deleteIntQuery } from '@/api/server'
  ...
  methods: {
      ...
      function treefiy (root) {
        if (!root.from.length && !root.to.length) {
          return {
            ...
            attrAliases: root.node.attrs ? root.node.attrs.map((_, index) => (_.name ? _.name : root.node.attrAliases[index])) : []
          }
        }
        const t = {
          children: [],
    ...
    async saveGraph () {
      ...
      if (!reqData) {
        ...
        const { statusCode, message } = this.isNewIntQuery ? await saveIntQuery(this.selectedCI.id, this.newGraphName, reqData) : await updateIntQuery(this.selectedQuery.id, reqData)
        if (statusCode === 'OK') {
          this.$Notice.success({
            title: 'Success',
            desc: message
