<template>
  <div class="physical-graph">
    <div class="graph-list">
      <div v-for="item in graphData" class="graph-container" :key="item.guid" :id="`graph_${item.guid}`">
...
export default {
  ...
  methods: {
    ...
    genDOT (data) {
      ...
      let dots = ['digraph G{', 'rankdir=TB nodesep=0.5;', `node[shape=box;fontsize=${fontSize};labelloc=t;penwidth=2];`, `subgraph cluster_${data.guid} {`, `style="filled";color="${colors[0]}";`, `tooltip="${tooltip}";`, `label="${label}";`, this.genArea(data), this.genLink(data.guid), '}}']
    ...
    genArea (data) {
      ...
      if (data.children instanceof Array) {
        ...
        layers.forEach(layer => {
          ...
          let dots = ['{rank=same;', '}']
          layer.forEach(_ => {
            dots.splice(-1, 0, `g_${_.guid}`, `[id="g_${_.guid}";`, `label="${_.data.code || _.data.key_name}";`, `tooltip="${_.data.description || _.data.code}";`, `width="${lw}";`, `height="${lh}"]`)
          })
          result += dots.join('')
        })
      } else {
        const dots = ['{rank=same;', `g_${data.guid}`, `[id="g_${data.guid}";`, `label=" ";`, `color="${colors[0]}";`, `tooltip="${data.data.description || data.data.code}";`, `width="${width - 0.5}";`, `height="${height - 3}"]`, '}']
        result = dots.join('')
      }
      return result
    },
    genLink (guid) {
      let result = ''
      const linkData = (this.links.find(_ => _.idcGuid === guid) && this.links.find(_ => _.idcGuid === guid).linkList) || []
      linkData.forEach(_ => {
        if (_.data.network_zone_design_1) {
          if (this.idcs[guid].indexOf(_.data.network_zone_design_1.guid) >= 0 && this.idcs[guid].indexOf(_.data.network_zone_design_2.guid) >= 0) {
            result += `g_${_.data.network_zone_design_1.guid}->g_${_.data.network_zone_design_2.guid}[arrowhead=none];`
          }
        } else {
          if (this.idcs[guid].indexOf(_.data.network_zone_1.guid) >= 0 && this.idcs[guid].indexOf(_.data.network_zone_2.guid) >= 0) {
    ...
    setChildren (node, p1, pw, ph, tfsize, level, rootGuid) {
      ...
      if (pw > ph * 1.2) {
        ...
        for (var i = 0; i < n; i++) {
          ...
          if (Array.isArray(node.children[i].children)) {
            this.setChildren(node.children[i], { x: rx, y: ry }, w, h, fontsize, level + 1, rootGuid)
          }
        }
      } else {
        if ((ph - tfsize) / n > pw) {
        ...
        for (var p = 0; p < n; p++) {
          rx = p1.x + mgap
          ry = p1.y + tfsize + (h + mgap) * p + mgap
          tx = p1.x + w * 0.5 + mgap
          if (Array.isArray(node.children[p].children)) {
            ty = p1.y + tfsize + (h + mgap) * p + fontsize + mgap
          } else {
            ty = p1.y + tfsize + (h + mgap) * p + h * 0.5 + mgap
          }

          g = graph
            .append('g')
            .attr('class', 'node')
            .attr('id', `g_${node.children[p].guid}`)
          g.append('title').text(node.children[p].data.description || node.children[p].data.code)
          g.append('rect')
            .attr('x', rx)
            .attr('y', ry)
            .attr('width', w)
          ...
          g.append('text')
            .attr('x', tx)
            .attr('y', ty)
            .text(node.children[p].data.code ? node.children[p].data.code : node.children[p].data.key_name)
            .attr('style', 'text-anchor:middle')
            .attr('font-size', fontsize)
          if (Array.isArray(node.children[p].children)) {
            this.setChildren(node.children[p], { x: rx, y: ry }, w, h, fontsize, level + 1, rootGuid)
          }
        }
      }
    },
    translateAndScale (_this, guid, divWidth, divHeight) {
      ...
      graph0.attr('transform', 'translate(' + translateX + ', ' + translateY + ') scale(' + scale + ')').on('click', function () {
