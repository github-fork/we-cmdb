...
import { getAllIdcData, getIdcImplementTreeByGuid, getResourcePlanningCiData, getCiTypeAttributes, deleteCiDatas, createCiDatas, updateCiDatas, getEnumCodesByCategoryId, getResourcePlanningTabs, getAllZoneLinkGroupByIdc, operateCiState } from '@/api/server'
...
export default {
  ...
  methods: {
    ...
    genDOT (idcData, linkData) {
      ...
      const width = 16
      const height = 12
      const dots = ['digraph G {', 'rankdir=TB nodesep=0.5;', `node [shape="box", fontsize="${fsize}", labelloc="t", penwidth="2"];`, `subgraph cluster_${idcData.data.guid} {`, `style="filled";color="${colors[0]}";`, `label="${idcData.data.name || idcData.data.description || idcData.data.code}";`, `size="${width},${height}";`, this.genChildren(idcData), this.genLink(links), '}}']
      return dots.join('')
    },
    genChildren (idcData) {
      const width = 16
