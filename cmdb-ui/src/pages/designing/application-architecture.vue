    ...
    <Row>
      <Col span="12">
        <Row>
          ...
          <Select filterable @on-change="onSystemDesignSelect" label-in-name style="width: 35%;">
            <Option v-for="item in systemDesigns" :value="item.guid" :key="item.guid">{{ item.name }}</Option>
          </Select>
          <Button style="margin: 0 10px;" @click="onArchChange">{{ $t('architecture_change') }}</Button>
    ...
    <Card style="margin-top: 20px">
      <div>
        <Tabs type="card" :value="currentTab" :closable="false" @on-click="handleTabClick">
          <TabPane :label="$t('application_logic_diagram')" name="architecture-design">
              ...
              <Col span="6" offset="0" class="func-wrapper" v-if="isShowInvokeSequence">
                <Collapse>
                  <Panel name="invokeDesignSequence">
                    ...
                    <Form slot="content" ref="invokeSequenceForm" :model="invokeSequenceForm" label-position="left" :label-width="110">
                      <Form-item :label="$t('invoking_sequential_design')">
                        <Row>
                          <Select :placeholder="$t('select_placeholder')" v-model="invokeSequenceForm.selectedInvokeSequence" style="width:calc(100% - 80px)">
                            <Option v-for="item in invokeSequenceForm.invokeSequenceData" :key="item.data.guid" :value="item.data.guid">{{ item.data.code }}</Option>
                          </Select>
                          <Button style="float:right;width:70px;" @click="onSearchInvokeSquence">{{ $t('confirm') }}</Button>
                        </Row>
                      </Form-item>
                      <Form-item :label="$t('invoking_sequential_design_sequence')" v-if="invokeSequenceForm.isShowInvokeSequenceDetial">
                      ...
                      <Form-Item v-if="invokeSequenceForm.isShowInvokeSequenceDetial">
                        <Button style="margin-right: calc(50% + 35px);width=70px;float: right" @click="closeInvokeSquence">{{ $t('back') }}</Button>
                      </Form-Item>
                    </Form>
                  </Panel>
                </Collapse>
          ...
          <TabPane :label="$t('physical_deployment_diagram')" name="physicalGraph">
...
import { getCiTypeAttributes, deleteCiDatas, createCiDatas, updateCiDatas, getEnumCodesByCategoryId, getSystemDesigns, updateCIRecord, getAllDesignTreeFromSystemDesign, saveAllDesignTreeFromSystemDesign, getArchitectureDesignTabs, getArchitectureCiDatas, getAllCITypes, operateCiState, getApplicationFrameworkDesignDataTree, getAllZoneLinkDesignGroupByIdcDesign } from '@/api/server'
  ...
  methods: {
    ...
    backInvokeSequence () {
      if (this.invokeSequenceForm.currentNum > 1) {
        ...
        this.invokeSequenceForm.currentInvokeSequenceTag = invokeDesignSequenceList[this.invokeSequenceForm.currentNum - 1].key_name
    ...
    onSearchInvokeSquence () {
      ...
      const found = this.invokeSequenceForm.invokeSequenceData.find(i => i.data.guid === this.invokeSequenceForm.selectedInvokeSequence)
      if (found) {
        this.invokeSequenceForm.currentInvokeSequence = found.data.invoke_design_sequence
    ...
    reColorInvokeSequence () {
      ...
      this.invokeSequenceForm.currentInvokeSequence.forEach(_ => {
        this.reColorCurrentInvokeSequenceNode(_.service_design, 'ellipse', 'green')
        this.reColorCurrentInvokeSequenceNode(_.unit_design, 'polygon', 'green')
    ...
    async getAllDesignTreeFromSystemDesign () {
      ...
      if (statusCode === 'OK') {
        ...
        this.systemDesignData = data || []
        this.initGraph()
      }

      await updateCIRecord(this.systemDesignCiTypeId, {
        guid: this.systemDesignVersion
      })
    },
    async getPhysicalGraphData () {
      this.physicalGraphData = []
      const promiseArray = [getApplicationFrameworkDesignDataTree(this.systemDesignVersion), getAllZoneLinkDesignGroupByIdcDesign()]
    ...
    genDOT (sysData) {
      ...
      const width = 16
      const height = 12
      let dots = ['digraph G{', 'rankdir=LR nodesep=0.5;', 'Node [fontname=Arial, fontsize=12];', 'Edge [fontname=Arial, minlen="2",fontsize=10];', `size="${width},${height}";`, `subgraph cluster_${sysData[0].guid} {`, `style="filled";color="${colors[0]}";`, `tooltip="${sysData[0].data.description}";`, `label="${sysData[0].data.name}";`, this.genChildrenDot(sysData[0].children || [], 1), '}', ...this.invokeLines, this.renderOtherNodes(), '}']
      ...
      if (data.length) {
        data.forEach(_ => {
          if (_.children instanceof Array && _.children.length) {
            dots = dots.concat([`subgraph cluster_${_.guid}{`, `id="g_${_.guid}";`, `style="filled";color="${colors[level]}";`, `label="${_.data.code || _.data.key_name}";`, `tooltip="${_.data.description || _.data.name}"`, _.ciTypeId === 3 ? this.genServiceInvokeLine(_) : this.genChildrenDot(_.children, level + 1), '}'])
          } else {
            this.physicalGraphNodes[_.guid] = _
            dots = dots.concat([`"${_.guid}"`, `[id="n_${_.guid}";`, `label="${_.data.code || _.data.key_name}";`, 'shape=box;', `style="filled";color="${colors[level]}";`, `tooltip="${_.data.description || _.data.name}"];`])
          }
        })
      } else {
    ...
    genServiceInvokeLine (unitNode) {
      ...
      if (InvokeLine.length) {
        InvokeLine.forEach(line => {
          ...
          this.invokeLines.push(`gn_${line.data.unit_design.guid} -> gn_${line.data.service_design.guid} [id="gl_${line.guid}",color="${color}",taillabel="${line.data.type.value}", labeldistance=3];`)
          this.physicalGraphLineNodes.serviceDesign[line.data.service_design.guid] = line.data.service_design
          this.physicalGraphLineNodes.unitDesign[line.data.unit_design.guid] = line.data.unit_design
        })
      }
      return [this.renderNode(unitNode, 'doubleoctagon'), ...this.renderServiceNode(serviceData, unitNode)].join('')
    },
    renderNode (node, nodeType) {
      this.physicalGraphNodes[node.guid] = node
      return `"gn_${node.guid}" [label="${node.data.code}", id="gn_${node.guid}", shape=${nodeType || 'ellipse'},style="filled", color="${stateColorMap.get(node.data.state.code)}", tooltip="${node.data.description || node.data.name}"]`
    },
    renderServiceNode (serviceData, unitNode) {
      let result = []
      if (serviceData.length) {
        serviceData.forEach(_ => {
          result = result.concat([this.renderNode(_), `"gn_${_.guid}" -> "gn_${unitNode.guid}" [arrowhead = "t"]`])
        })
      }
      return result
    },
    renderOtherNodes () {
      let result = ''
      Object.keys(this.physicalGraphLineNodes.serviceDesign).forEach(serviceGuid => {
        if (!this.physicalGraphNodes[serviceGuid]) {
          const node = this.physicalGraphLineNodes.serviceDesign[serviceGuid]
          result += `"gn_${node.guid}" [label="${node.code}",id="gn_${node.guid}",shape="ellipse",tooltip="${node.description || node.name}"]`
        }
      })
      Object.keys(this.physicalGraphLineNodes.unitDesign).forEach(unitGuid => {
        if (!this.physicalGraphNodes[unitGuid]) {
          ...
          result += `"gn_${node.guid}" [label="${node.code}",id="gn_${node.guid}",shape="doubleoctagon",tooltip="${node.description || node.name}"]`
        }
      })
      return result
    },
    ...
    async exportHandler () {
      ...
      const { statusCode, data } = await getArchitectureCiDatas(this.currentTab, this.systemDesignVersion, exportPayload)
      if (statusCode === 'OK') {
      ...
      if (statusCode === 'OK') {
        ...
        data.forEach(_ => {
          ...
          if (_.status !== 'decommissioned' && _.status !== 'notCreated') {
            const com = _.propertyName === 'invoke_design_sequence' ? { component: 'WeCMDBSequenceDiagram' } : { ...components[_.inputType] }
            columns.push({
              ..._,
              title: _.name,
              key: renderKey,
    ...
    onSystemDesignSelect (key) {
      ...
      if (this.currentTab !== 'architecture-design' && this.currentTab !== 'physicalGraph') {
        this.getCurrentData()
      }
    },
