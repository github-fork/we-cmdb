  ...
  <Row>
    <Col span="4">
      <Card>
        ...
        <Tag v-for="item in users" :key="item.id" :name="item.username" :color="item.color" :checked="item.checked" checkable :fade="false" @on-change="handleUserClick">
          <span :title="` ${item.username} ( ${item.description} ) `">{{ ` ${item.username} ( ${item.description} ) ` }}</span>
        </Tag>
      </Card>
    </Col>
    <Col span="5" offset="0" style="margin-left: 20px">
      <Card>
        ...
        <div class="role-item" v-for="item in roles" :key="item.id">
          <Tag :name="item.id" :color="item.color" :checked="item.checked" checkable :fade="false" @on-change="handleRoleClick">
            <span :title="item.description">{{ item.description }}</span>
          </Tag>
          <Button icon="ios-build" type="dashed" size="small" @click="openUserManageModal(item.id)">{{ $t('user') }}</Button>
        </div>
      </Card>
    </Col>
      ...
      <Card>
        ...
        <div class="data-permissions" v-for="ci in ciTypePermissions" :key="ci.ciTypeId">
          ...
          <div class="ciTypes-options">
            <Checkbox v-for="act in actionsType" :disabled="dataPermissionDisabled" :indeterminate="ci[act.actionCode] === 'P'" :value="ci[act.actionCode] === 'Y'" :key="act.actionCode" @click.prevent.native="ciTypesPermissionsHandler(ci, act.actionCode, act.type)">{{ act.actionName }}</Checkbox>
          </div>
        </div>
      </Card>
    </Col>
    ...
    <Modal v-model="userManageModal" width="700" :title="$t('edit_user')" @on-ok="confirmUser" @on-cancel="confirmUser">
      <Transfer :titles="transferTitles" :list-style="transferStyle" :data="allUsersForTransfer" :target-keys="usersKeyBySelectedRole" :render-format="renderUserNameForTransfer" @on-change="handleUserTransferChange" filterable></Transfer>
    </Modal>
    <Modal v-model="permissionManageModal" :title="$t('edit_data_authority')" @on-ok="cancelEdit" @on-cancel="cancelEdit" width="80">
      <WeCMDBTable :tableData="ciTypeAttrsPermissions" :filtersHidden="true" :tableOuterActions="outerActions" :tableInnerActions="innerActions" :tableColumns="attrsPermissionsColumns" :ascOptions="permissionsTableOptions" :showCheckbox="true" @actionFun="actionFun" @getSelectedRows="onSelectedRowsChange" tableHeight="650" ref="table"></WeCMDBTable>
    </Modal>
  </Row>
</template>
<script>
import { outerActions, innerActions, components } from '@/const/actions.js'
import { getAllUsers, getAllRoles, getAllMenus, getRolesByUser, getUsersByRole, addRole, addUser, getPermissionsByRole, addUsersToRole, romoveUsersFromRole, getPermissionsByUser, addMenusToRole, removeMenusFromRole, addDataPermissionAction, removeDataPermissionAction, getRoleCiTypeCtrlAttributesByRoleCiTypeId, getEnumCodesByCategoryId, createRoleCiTypeCtrlAttributes, updateRoleCiTypeCtrlAttributes, deleteRoleCiTypeCtrlAttributes } from '@/api/server.js'
...
export default {
  data () {
    return {
      ...
      allUsersForTransfer: [],
      selectedRole: 0,
      transferTitles: [this.$t('permission_management_unselected_user'), this.$t('permission_management_selected_user')],
      currentRoleId: 0,
      ciTypePermissions: [],
  ...
  methods: {
        ...
        this.ciTypeAttrsPermissions = data.body.map(_ => {
          ...
          for (let i in _) {
            ...
            if (typeof _[i] === 'object' && _[i] !== null) {
              obj[i] = {
                codeId: _[i].conditionValue && isRef ? _[i].conditionValue : _[i].conditionValue.split(',').map(n => n * 1),
                value:
    ...
    async saveHandler (data) {
      ...
      const defaultKey = ['creationPermission', 'enquiryPermission', 'executionPermission', 'grantPermission', 'modificationPermission', 'removalPermission', 'roleCiTypeId', 'roleCiTypeCtrlAttrId', 'callbackId']
      if (addAry.length > 0) {
        addAry.forEach(_ => {
    ...
    async handleRoleClick (checked, id) {
      ...
      let { statusCode, data } = await getUsersByRole(id)
    ...
    async handleMenuTreeCheck (allChecked, currentChecked) {
      ...
      const { statusCode, message } = currentChecked.checked ? await addMenusToRole(menuCodes, this.currentRoleId) : await removeMenusFromRole(menuCodes, this.currentRoleId)
      if (statusCode === 'OK') {
        this.$Notice.success({
          title: 'success',
          desc: message
    ...
    async setPermissionAction (checked, name, id) {
      ...
      if (checked) {
        const addRes = await addDataPermissionAction(this.currentRoleId, id, name)
        this.permissionResponseHandeler(addRes)
      } else {
        const delRes = await removeDataPermissionAction(this.currentRoleId, id, name)
