<template>
  <WeCMDBTable :tableData="tableData" :tableOuterActions="outerActions" :tableInnerActions="innerActions" :tableColumns="tableColumns" :pagination="pagination" :ascOptions="ascOptions" :showCheckbox="showCheckbox" :isSortable="false" @actionFun="actionFun" @getSelectedRows="onSelectedRowsChange" @handleSubmit="handleSubmit" @sortHandler="sortHandler" @pageChange="pageChange" @pageSizeChange="pageSizeChange" @getGroupList="getGroupList" tableHeight="650" ref="table"></WeCMDBTable>
</template>

<script>
import { getAllSystemEnumCodes, getSystemCategories, getEffectiveStatus, createEnumCode, updateEnumCode, getGroupListByCodeId, deleteEnumCodes, getAllNonSystemEnumCodes, getNonSystemCategories } from '@/api/server.js'
...
export default {
  ...
  methods: {
    ...
    async queryData () {
      ...
      const { statusCode, data } = this.$route.name === 'baseData' ? await getAllSystemEnumCodes(this.payload) : await getAllNonSystemEnumCodes(this.payload)
      if (statusCode === 'OK') {
        ...
        this.tableData = data.contents.map(_ => {
          return {
            ..._,
            ..._.cat,
            catTypeName: _.cat.catType.catTypeName === 'sys' ? this.$t('system_enum') : _.cat.catType.catTypeName === 'common' ? this.$t('common_enum') : `${this.$t('private_enum')}-${_.cat.catType.catTypeName}`
          }
        })
      }
    },
    async getEnumNames () {
      const { statusCode, data } = this.$route.name === 'baseData' ? await getSystemCategories() : await getNonSystemCategories()
      if (statusCode === 'OK') {
    ...
    async exportHandler () {
      const { statusCode, data } = this.$route.name === 'baseData' ? await getAllSystemEnumCodes({ ...this.payload, paging: false }) : await getAllNonSystemEnumCodes({ ...this.payload, paging: false })
