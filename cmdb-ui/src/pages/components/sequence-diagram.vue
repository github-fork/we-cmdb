<template>
  <div>
    ...
    <Modal v-model="sequenceVisiable" @on-visible-change="visibleChangeHandler" width="90" :footer-hide="true" :mask-closable="false" :title="$t('invoking_design')">
      <Row>
        <Col span="17">
          <Card>
        ...
        <Col span="7" style="padding: 0 0 0 20px">
          <Card>
            ...
            <Row>
              ...
              <Col span="16">
                <Select multiple filterable @on-change="selectChangeHandler" :max-tag-count="1" clearabled v-model="selectedSequences">
                  <Option v-for="s in sequenceData" :label="s.key_name" :value="s.guid" :key="s.guid"></Option>
                </Select>
              </Col>
            </Row>
            <p v-for="sequence in selectedSequenceData" style="display: flex;margin-top:5px;border: 1px dashed gray;border-radius: 5px;padding: 0 10px;" :key="sequence.guid">
              <span class="sequence_name" :title="sequence.key_name">{{ sequence.key_name }}</span>
              <Button size="small" @click.stop.prevent="moveUpSequence(sequence.guid)" icon="md-arrow-round-up"></Button>
              <Button size="small" @click.stop.prevent="moveDownSequence(sequence.guid)" icon="md-arrow-round-down"></Button>
            </p>
          </Card>
        </Col>
      </Row>
    ...
    initSequenceGraph () {
      ...
      if (this.selectedSequenceData.length > 0) {
        this.selectedSequenceData.forEach((_, index) => {
          graphNode.push(`${_.unit_design.key_name.replace(/-/g, '&')}->>${_.service_design.key_name.replace(`-${_.service_design.code}`, '').replace(/-/g, '&')}:${index + 1} : ${_.description.length === 0 ? '' : _.description}`)
        })
