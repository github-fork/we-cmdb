...
export default {
  ...
  watch: {
    ...
    routine (val) {
      ...
      if (attrId) {
        if (this.ciTypeAttributeObj[attrId].inputType === 'ref' || this.ciTypeAttributeObj[attrId].inputType === 'multiRef') {
  ...
  methods: {
      ...
      if (this.cmdbColumnSource && this.ciTypeAttributeObj) {
        ...
        this.routine.forEach((_, i) => {
          if (i > 0) {
            ...
            const newVal = this.ciTypeAttributeObj[_.ciTypeAttrId].inputType === 'ref' ? `(${ciTypeAttrName})${ciTypeName} ` : ciTypeAttrName + ' '
          ...
          if (statusCode === 'OK') {
            ...
            data.forEach(_ => {
              if (_.status === 'created') {
                attr.push({
                  ..._,
                  ciTypeName: _.inputType === 'ref' ? this.allCi.find(i => i.ciTypeId === _.referenceId).name : this.allCi.find(i => i.ciTypeId === _.ciTypeId).name,
            ...
            this.options = this.isEndWithCIType ? attr.filter(_ => _.inputType === 'ref' || _.inputType === 'multiRef') : attr
          } else {
            this.$Message.error({
              content: message
            })
    ...
    optClickHandler (item) {
      ...
      const newValue = item.inputType === 'ref' ? `(${item.ciTypeAttrName})${item.ciTypeName} ` : item.ciTypeAttrName + ' '
