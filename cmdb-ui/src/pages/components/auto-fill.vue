        ...
        <div class="attr-ul">
          ...
          <ul v-for="opt in attrNameArray" :key="opt.attrId">
            <li @click="selectAttr(opt)">
              {{ opt.inputType === 'ref' || opt.inputType === 'multiRef' ? `( ${opt.ciTypeAttrName} ) ${opt.ciTypeName}` : opt.ciTypeAttrName }}
            </li>
          </ul>
...
export default {
  ...
  methods: {
      ...
      if (this.inputRuleStatus === 1) {
        ...
        if (v.data) {
          if (!this.autoFillLastObjValue) {
            ...
            if (!obj.parentRs || this.ciTypeAttrsObj[obj.parentRs.attrId].inputType === 'ref' || this.ciTypeAttrsObj[obj.parentRs.attrId].inputType === 'multiRef') {
              if (v.data === '.' || v.data === '-') {
                ...
                this.$refs.textarea.value = this.inputVal
                this.$Message.error({
                  content: this.$t('auto_fill_legitimate_operation_character_tips')
                })
              }
            } else if (!obj.parentRs || ((this.ciTypeAttrsObj[obj.parentRs.attrId].inputType === 'select' || this.ciTypeAttrsObj[obj.parentRs.attrId].inputType === 'multiSelect') && !obj.enumCodeAttr)) {
              this.$refs.textarea.value = this.inputVal
              this.$Message.error({
          ...
          if (this.autoFillLastObjValue !== '[]' && this.autoFillLastObjValue !== '') {
            ...
            if (lastAttrObj.parentRs) {
              const attrName = this.ciTypeAttrsObj[lastAttrObj.parentRs.attrId].name
              lastAttrVal += lastAttrObj.parentRs.isReferedFromParent === 1 ? '.' : '-'
              if (this.ciTypeAttrsObj[lastAttrObj.parentRs.attrId].inputType === 'ref' || this.ciTypeAttrsObj[lastAttrObj.parentRs.attrId].inputType === 'multiRef') {
            ...
            this.inputVal = this.inputVal.substr(0, this.inputVal.lastIndexOf(lastAttrVal))
            val.splice(-1, 1)
            this.autoFillArray[this.autoFillArray.length - 1].value = JSON.stringify(val)
    ...
    setAutoData () {
      ...
      this.autoFillArray[this.autoFillArray.length - 1].value = val[val.length - 1]
    },
      ...
      if (operator === '.') {
        ...
        if (statusCode === 'OK') {
          data.forEach(_ => {
            attrArray.push({
              ..._,
              ciTypeName: _.inputType === 'ref' || _.inputType === 'multiRef' ? this.allCiTypes.find(i => i.ciTypeId === _.referenceId).name : this.allCiTypes.find(i => i.ciTypeId === _.ciTypeId).name,
    ...
    selectAttr (opt) {
      ...
      result.push(val)
      this.autoFillArray[this.autoFillArray.length - 1].value = JSON.stringify(result)
      this.inputVal += opt.inputType === 'ref' || opt.inputType === 'multiRef' ? `(${opt.ciTypeAttrName})${opt.ciTypeName} ` : opt.ciTypeAttrName + ' '
      this.attrNameArray = []
      this.$refs.textarea.focus()
    ...
    displayInputData () {
      ...
      if (this.autoFillArray[this.autoFillArray.length - 1].type !== 'delimiter') {
        this.autoFillArray.push({ type: 'delimiter', value: '' })
      }
        ...
        if (_.type === 'delimiter') {
          ...
          data.forEach(item => {
            ...
            if (item.parentRs) {
              ...
              if (this.ciTypeAttrsObj[item.parentRs.attrId].inputType === 'ref' || this.ciTypeAttrsObj[item.parentRs.attrId].inputType === 'multiRef') {
                val += `${refType}(${attrName})${ciTypeName} `
              } else if (this.ciTypeAttrsObj[item.parentRs.attrId].inputType === 'select' || this.ciTypeAttrsObj[item.parentRs.attrId].inputType === 'multiSelect') {
