<template>
  <div class="container">
    ...
    <Input style="width:150px" v-else :placeholder="$t('please_select_ci_type')" @on-change="rootChange" :value="allCmdbAttrs[0].val"></Input>
    <Select
      :style="{
        'border-radius': '4px',
        width: '150px',
    ...
    >
      <Option v-for="opt in attr.options" :key="opt.attrId" :value="opt.refName">{{ `${opt.ciTypeName} ( ${opt.refName} )` }}</Option>
    </Select>
  </div>
</template>
<script>
          ...
          if (_.t === 'attr') {
            ...
            if (statusCode === 'OK') {
              opt = data.map(p => {
                return {
                  ...p,
                  ciTypeName: p.inputType === 'ref' ? this.allCi.find(i => i.ciTypeId === p.referenceId).name : this.allCi.find(i => i.ciTypeId === _.id).name,
                  refName: p.name,
                  ciTypeId: _.id,
        ...
        if (operator === '.') {
          ...
          if (statusCode === 'OK') {
            const opt = data.map(_ => {
              return {
                ..._,
                ciTypeName: _.inputType === 'ref' ? this.allCi.find(i => i.ciTypeId === _.referenceId).name : this.allCi.find(i => i.ciTypeId === found.ciTypeId).name,
                refName: _.name,
                ciTypeId: id,
