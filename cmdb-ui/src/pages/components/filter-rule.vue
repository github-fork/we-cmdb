          ...
          if (v.data === '{') {
            if (!this.isLeftFilterRuleStart) {
              if (this.filterAttr.inputType === 'ref' || this.filterAttr.inputType === 'multiRef') {
                ...
                this.lastCiTypeId = this.filterAttr.inputType === 'ref' || this.filterAttr.inputType === 'multiRef' ? this.filterAttr.referenceId : this.filterAttr.ciTypeId
        ...
        if (v.data) {
          if (this.isInputStartWithArray === true) {
            ...
            if (rule.test(v.data)) {
              ...
              this.isLeftFilterRule = false
              this.inputVal = this.$refs.textarea.value
              let groupInfo = this.inputVal.substring(this.inputVal.lastIndexOf('[') + 1, this.inputVal.lastIndexOf(']')).split(',')
    ...
    async getNextRef (operator) {
      ...
      if (operator === '.') {
        if (this.lastCiTypeId !== 0) {
          ...
          if (statusCode === 'OK') {
            if (this.isLeftFilterRule === true && objList.length === 2 && (this.ciTypeAttrsObj[objList[1].parentRs.attrId].inputType === 'ref' || this.ciTypeAttrsObj[objList[1].parentRs.attrId].inputType === 'multiRef')) {
              data.forEach(_ => {
            ...
            filterLeftRule.forEach((item, index) => {
              if (item.parentRs) {
                ...
                if (index === 1) {
                  ...
                  if (this.ciTypeAttrsObj[item.parentRs.attrId].inputType === 'select' || this.ciTypeAttrsObj[item.parentRs.attrId].inputType === 'multiSelect') {
                    val += `.${attrName} .${item.enumCodeAttr} `
                  } else {
                    val += `.${attrName} `
                  }
